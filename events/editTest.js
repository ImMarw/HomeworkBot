...
module.exports =  {
    ...
    async execute(interaction, args) {
        let testData = null;
        db.all(`SELECT * FROM test WHERE id = ?`, [args[0]], async (err, rows) => {
            if (err) {
                console.error(err.message);
                interaction.reply({content: 'An error occurred while fetching test data.', ephemeral: true});
                return;
            }
            ...
            if (rows.length == 1) {
                ...
                const modal = new ModalBuilder()
                    .setCustomId('testeditmodal_' + args[0])
                    .setTitle('Edit ');
                ...
                console.log('No test found with the given ID.');
                interaction.reply({content: 'No test found with the given ID.', ephemeral: true});
                return;
            }
        });
    }
