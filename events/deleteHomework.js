...
module.exports =  {
    ...
    async execute(interaction, args) {
        ...
        db.run(`UPDATE homework set message_id = (NULL) WHERE ID = ?`, [args[0]], async (err) => {
            if (err) {
                console.error(err.message);
                interaction.reply({content: 'An error occurred while deleting homework.', ephemeral: true});
                return;
            }
