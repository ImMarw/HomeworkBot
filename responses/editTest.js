...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            db.run(
                'UPDATE test SET subject = ?, topic = ?, description = ?, due_date = ? WHERE id = ?',
                [subject.toUpperCase(), topic.capitalize(), description.capitalize(), formattedDate, testId],
                async function (err) {
                    if (err) {
                    ...
                    db.all('SELECT message_id FROM test WHERE id = ?', [testId], async (err, result) => {
                        if (err) {
