...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('test')
        .setNameLocalizations({ 'cs': 'test' })
        .setDescription('Adds a new test')
        .setDescriptionLocalizations({ cs: 'Přidá nový test' })
        .addStringOption(opt =>
            opt
                .setName('subject')
        ...
        .addStringOption(opt =>
            opt
                .setName('topic')
                .setDescription('Title of the test')
                .setDescriptionLocalizations({ cs: 'Nadpis testu' })
                .setRequired(true)
        )
        .addStringOption(opt =>
            opt
                .setName('description')
                .setDescription('Details about the test')
                .setDescriptionLocalizations({ cs: 'Detaily ohledně testu' })
                .setRequired(true)
        )
        .addStringOption(opt =>
            opt
    ...
    async execute(interaction) {
        ...
        db.run(
            'INSERT INTO test (subject, topic, description, due_date, user_id) VALUES (?, ?, ?, ?, ?)',
            [subject.toUpperCase(), topic.capitalize(), description.capitalize(), formatted, interaction.user.id],
            async function (err) {
                if (err) {
                    console.error('DB error:', err.message);
                    return interaction.reply({ content: '❌ Nepodařilo se přidat test.', ephemeral: true });
                }
                ...
                const editButton = new ButtonBuilder()
                    .setCustomId(`testedit_${testId}`)
                    .setEmoji('✏️')
                    .setLabel('Upravit')
                    .setStyle(ButtonStyle.Primary)

                const deleteButton = new ButtonBuilder()
                    .setCustomId(`testdelete_${testId}`)
                    .setLabel('🗑️ Smazat')
                ...
                if (!channel) {
                    return interaction.reply({ content: '❌ Kanál pro testy nebyl nalezen.', ephemeral: true });
                }

                const message = await channel.send({ embeds: [embed], components: [row] });

                db.run('UPDATE test SET message_id = ? WHERE id = ?', [message.id, testId]);

                return interaction.reply({ content: `✅ test byl přidán do kanálu ${channel}`, ephemeral: true });
            }
        );
    },
};
